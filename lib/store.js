...
const { toMysqlValue, fromMysqlValues } = require('./helpers');
  ...
  _get(model, pk) {
    ...
    return this._filter(model, query).then((result) => {
      if (result && result.length > 0) {
        return fromMysqlValues(model.schema.schema, result[0]);
      }
  ...
  _filter(model, query) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._pool.query({ sql, timeout }, (err, result) => {
        ...
        return resolve((result || []).map(el => fromMysqlValues(model.schema.schema, el)));
      });
    });
  }
